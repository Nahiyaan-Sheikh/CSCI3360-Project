...
ax2.scatter(x_trainArr, y_trainArr, color='black', alpha=0.5)
ax2.plot(x_trainArr, regBTC.predict(x_train), color='blue', linewidth=2)
theta = np.polyfit(x_trainArr, y_trainArr, 1)
y_line = theta[1] + theta[0] * x_trainArr
ax2.plot(x_trainArr, y_line, color='green', linewidth=2)
plt.show()
...
x_train13, x_test13, y_train13, y_test13 = train_test_split(cryptoArr, reg_df['rx_580_price'], test_size=0.25, random_state=0)
...
xArr = np.array(x_train13)[:,0].astype(float)
yArr = np.array(y_train13).astype(float)

print('')
print('xArr: ', xArr.size)
print('yArr: ', yArr.size)
